...
class MSSQLDatabaseTools(DatabaseTools):
    ...
    def __init__(self, server, database, uid = '', pwd = '', trusted_connection = True):
        ...
        try:
            if (len(self.uid) > 0 or len(self.pwd) > 0) and self.trusted_connection:
                raise InputError('ERROR - AUTHENTICATION: Cannot use Windows Authentication if user name and password is provided')
    ...
    def connect_to_db(self):
        ...
        if self.trusted_connection:
            cnxn = db.connect(r'Driver={' + self.driver + '};Server=' + self.server + ';Database=' + self.database + ';Trusted_Connection=yes;')
        else:
    ...
    def bcp_from_df(self, df, table_name, drop_if_exist = True, batch_size=10000):
        ...
        if self.trusted_connection:
            ...
            sql_config = {'server': self.server,'database': self.database,'username': self.uid,'password':self.pwd}
        ...
        bdf.to_sql(sql_table, use_existing_sql_table=self.is_table_exists(table_name), batch_size=batch_size)
    ...
    def bcp_from_file(self, file_name, table_name, delimiter = ',', drop_if_exist = True, batch_size=10000):
        ...
        self.bcp_from_df(df = df, table_name = table_name, drop_if_exist = drop_if_exist, batch_size=batch_size)
    ...
    def bcp_from_gzip(self, file_name, table_name, delimiter = ',', drop_if_exist = True, batch_size=10000):
        ...
        self.bcp_from_df(df, table_name, drop_if_exist = drop_if_exist, batch_size=batch_size)
