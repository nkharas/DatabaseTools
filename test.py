...
def bcp_data():
    ...
    dbconn.bcp_from_file('test_data\\green_tripdata_2019-01_subset.csv', 'green_cab_trips', delimiter='|')
...
def filtered_download():
    dbconn = MSSQLDatabaseTools(server, database, uid, pwd, False)
    df = dbconn.download_from_table('green_cab_trips', where_clause = 'VendorID = 2')
...
def bcp_data_append():
    dbconn = MSSQLDatabaseTools(server, database, uid, pwd, False)
    dbconn.bcp_from_file('test_data\\green_tripdata_2019-01_subset.csv', 'green_cab_trips', delimiter='|', drop_if_exist = False)
